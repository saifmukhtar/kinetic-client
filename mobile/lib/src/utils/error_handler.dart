...
String parseKineticError(dynamic error) {
  if (error is ResolverError) {
    return error.when(
      ...
      notFound: (name) =>
          "Name '$name' was not found in the Kinetic network. It may be unregistered.",
      expired: (name, rounds) =>
          "The registration for '$name' has expired ($rounds rounds old).",
      invalidUrl: (url) => "Invalid URL format: $url",
      noWebsiteService: (kid) =>
          "No 'website' service found in manifest for KID $kid",
    ...
    return error.when(
      ...
      offline: () =>
          "You appear to be offline. Cannot connect to the Kinetic network.",
      notFound: (name) =>
          "Identity '$name' was not found in the Kinetic network.",
      internal: (msg) => "Internal error: $msg",
    );
  }
