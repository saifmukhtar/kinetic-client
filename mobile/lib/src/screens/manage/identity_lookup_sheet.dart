...
class IdentityLookupSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<IdentityLookupSheet> createState() =>
      _IdentityLookupSheetState();
}
...
class _IdentityLookupSheetState extends ConsumerState<IdentityLookupSheet> {
  ...
  Widget _buildResultCard(
    ...
    final formattedName = isDirectNpub
        ? name
        : (name.endsWith(AppConstants.dotTld)
              ? name
              : '$name${AppConstants.dotTld}');
    final status = data['status'] as String? ?? 'Unknown';
    final isActive = status == 'Verified';

    final hasProfile =
        data['profile'] != null && (data['profile'] as Map).isNotEmpty;
...
class _ProfileHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final displayName =
        (profile['name'] is String ? profile['name'] : null) ??
        (profile['display_name'] is String ? profile['display_name'] : null);
    final nip05 = profile['nip05'] is String
        ? profile['nip05'] as String
        : null;
    final website =
        (profile['website'] is String ? profile['website'] : null) ??
        (profile['url'] is String ? profile['url'] : null);
    final github = profile['github'] is String
        ? profile['github'] as String
        : null;
    final twitter =
        (profile['twitter'] is String ? profile['twitter'] : null) ??
        (profile['x'] is String ? profile['x'] : null);
    final lud16 = profile['lud16'] is String
        ? profile['lud16'] as String
        : null;
    final picture = profile['picture'] is String
        ? profile['picture'] as String
        : null;
    final about = profile['about'] is String
              ...
              Wrap(
                ...
                children: [
                  if (website != null && website.isNotEmpty)
                    _SocialChip(icon: Icons.language_rounded, label: 'Website'),
                  if (github != null && github.isNotEmpty)
                    _SocialChip(icon: Icons.code_rounded, label: 'GitHub'),
                  ...
                  if (lud16 != null && lud16.isNotEmpty)
                    _SocialChip(icon: Icons.bolt_rounded, label: 'Lightning'),
                ],
              ),
            ],
          ),
